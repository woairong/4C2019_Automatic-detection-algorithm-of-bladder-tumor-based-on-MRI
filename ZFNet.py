...
class History(keras.callbacks.Callback):
    ...
    def make_remark(self):
        ...
        wb.save('./excel/plot_data_ZFNet.xlsx')
    ...
    def loss_plot(self, loss_type):
        ...
        plt.savefig('./image/train-acc-loss.png')
        plt.show()
        plt.clf()
        ...
        if loss_type == 'epoch':
            ...
            plt.legend(loc="upper right")
            plt.savefig('./image/val_acc-loss.png')
            plt.show()


def zfnet():
    model = Sequential()
    model.add(Convolution2D(96, (7, 7), strides=(2, 2), input_shape=(224, 224, 1), padding='valid', activation='relu', kernel_initializer='uniform'))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
    model.add(Convolution2D(256, (5, 5), strides=(2, 2), padding='same', activation='relu', kernel_initializer='uniform'))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
...
###################################################################################
# path的不同选择对应着不同的的批次实验

# 各个实验数据名称及其数据解释如下：
# '../DATA/TraingData1/': 第一批次数据实验》》“原始1320实验数据集”
# '../DATA/TraingData2/': 第二批次数据实验》》“裁剪后1320实验数据集”
# '../DATA/TraingData3/': 第三批次数据实验》》“第二个子样本集”
# '../DATA/TraingData4/': 第四批次数据实验》》“第二个子样本旋转集”

###################################################################################

path = '../DATA/TrainingData1/'
