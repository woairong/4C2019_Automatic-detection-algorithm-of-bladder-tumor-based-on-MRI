...
class History(keras.callbacks.Callback):
    ...
    def make_remark(self):
        path = './excel/plot_data_CNN_SVM.csv'
        file_csv = codecs.open(path, 'w+', 'utf-8')  # 追加
        writer = csv.writer(file_csv)
        writer.writerow(self.train_acc['batch'])
        writer.writerow(self.train_loss['batch'])
        writer.writerow(self.train_acc['epoch'])
        writer.writerow(self.train_loss['epoch'])
        writer.writerow(self.val_acc['epoch'])
        writer.writerow(self.val_loss['epoch'])
    ...
    def loss_plot(self, loss_type):
        ...
        if loss_type == 'epoch':
            ...
            plt.legend(loc="upper right")
            plt.savefig('./image/val-acc-loss.png')
            plt.show()
